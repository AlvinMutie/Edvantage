from app.models.risk import Intervention, InterventionOutcome, InterventionRecommendation, InterventionTemplate
...
class InterventionAnalyticsService:
    def record_outcome(self, intervention_id, outcome_data, completed_by_id):
        ...
        # engagement_after could be pulled from StudentEngagement
        
        # 2. Capture "Before" Snapshot 
        # Prefer metrics from the recommendation evidence if available
        gpa_before = None
        ...
        # Fallback if no recommendation link or missing evidence
        if risk_before is None: risk_before = risk_after # Simplified
    ...
    def _calculate_effectiveness(self, gpa_b, gpa_a, att_b, att_a, rating):
        # Heuristic: 40% rating, 30% GPA change, 30% Attendance change
        rating_score = (rating / 5.0) * 0.4
        
        gpa_change = (gpa_a - gpa_b) if (gpa_a and gpa_b) else 0
        gpa_score = max(0, min(0.3, (gpa_change / 1.0) * 0.3)) # Cap at 1.0 GPA improvement
        
        att_change = (att_a - att_b) if (att_a and att_b) else 0
        att_score = max(0, min(0.3, (att_change / 20.0) * 0.3)) # Cap at 20% attendance improvement
        
        return rating_score + gpa_score + att_score

    def get_summary_stats(self):
        """Returns institution-wide effectiveness stats"""
